load("R/application/eeg/con_eeg_model.RData")
...
spt.gradients_con = spt_gradients(model = model,
                                  cov.type = "matern2",
                                  grid.points = grid.shape,
                                  true = NULL, plots = FALSE)
save(spt.gradients_con, file = "R/application/eeg/con_gradients.RData")
do.call(rbind, lapply(spt.gradients_con, function(x) unlist(lapply(x, function(y) sum(y$signif)))))[,-1]
...
for(j in 1:17){
  ...
  for(i in 1:6){
    derivs = spt.gradients_con[[i]][[j + 2]][,1]
    col.y[[i]] = sapply(spt.gradients_con[[i]][[j + 2]]$signif, function(x){
      if(x == 0) return("grey")
...
pdf("R/application/eeg/full_grad_plot_eeg_con.pdf", width = 11, height = 5)
gg.arrange
dev.off()
