load("R/application/canada wildfires/model_sepoct.RData")
...
# spt.gradients_sepoct = spt_gradients(model = model,
#                                      cov.type = "matern2",
#                                      grid.points = grid.shape.points,
#                                      true = NULL, plots = FALSE)
# save(spt.gradients_sepoct, file = "R/application/canada wildfires/sepoct_gradients.RData")
load("R/application/canada wildfires/sepoct_gradients.RData")
do.call(rbind, lapply(spt.gradients_sepoct, function(x) unlist(lapply(x, function(y) sum(y$signif)))))[,-1]

gg.arrange = list()
for(j in 1:17){
  df.gg = col.y = list()
  for(i in 1:4){
    derivs = spt.gradients_sepoct[[i]][[j + 2]][,1]
    col.y[[i]] = sapply(spt.gradients_sepoct[[i]][[j + 2]]$signif, function(x){
      if(x == 0) return("grey")
  ...
  levels(df.gg.full$t) = c(paste("Sep. ", 29:30, ", 2023", sep = ""),
                           paste("Oct. ", 1:2, ", 2023", sep = ""))
...
pdf("R/application/canada wildfires/full_grad_plot_sepoct.pdf", width = 12, height = 5)
gg.arrange
dev.off()
