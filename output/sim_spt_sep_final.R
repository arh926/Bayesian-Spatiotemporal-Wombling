...
source("gibbs-mh-spt-sepbl.R")
source("spt_gradients-sep.R")
...
for(i in 1:length(Ns)){
  for(j in 1:length(Nt)){
    ...
    results.sim = mclapply(1:nrep, function(x){
      ...
      chain_y = hlmBayes_spt(coords = coords, t = t, y = y,
                             cov.type.s = cov.type.s, cov.type.t = cov.type.t,
                             niter = niter, nburn = nburn, report = report, verbose = TRUE)
    ...
    # save(results.sim, file = paste0("../data/sep/Simulation 1/ns-", Ns[i], "-nt-", Nt[j], "-reps.RData")) # pattern 1
    save(results.sim, file = paste0("../data/sep/Simulation 2/ns-", Ns[i], "-nt-", Nt[j], "-reps.RData")) # pattern 2
...
for(i in 1:length(Ns)){
  for(j in 1:length(Nt)){
    # load(paste0("../data/sep/Simulation 1/ns-", Ns[i], "-nt-", Nt[j], "-reps.RData"))
    load(paste0("../data/sep/Simulation 2/ns-", Ns[i], "-nt-", Nt[j], "-reps.RData"))
    t = 1:Nt[j]
    # True Values
    ...
    MSPE = CP = list()
    for(k in 1:nrep){
      spt.gradients = spt_gradients(model = results.sim[[k]], cov.type.s = cov.type.s, grid.points = grid.points, true = true, plots = FALSE)
    ...
    # save(mspe.cp.reps, file = paste0("../data/sep/Simulation 1/ns-", Ns[i], "-nt-", Nt[j], "-mspe-cp-reps.RData"))
    save(mspe.cp.reps, file = paste0("../data/sep/Simulation 2/ns-", Ns[i], "-nt-", Nt[j], "-mspe-cp-reps.RData"))
...
for(i in 1:length(Ns)){
  for(j in 1:length(Nt)){
    load(paste0("../data/sep/Simulation 1/ns-", Ns[i], "-nt-", Nt[j], "-mspe-cp-reps.RData"))
    # load(paste0("../data/Simulation 2/ns-", Ns[i], "-nt-", Nt[j], "-mspe-cp-reps.RData"))
    cp.results <- rbind(cp.results,
                        apply(do.call(rbind,lapply(mspe.cp.reps$CP, function(x) apply(x, 2, mean))), 2, mean))
    mean.results <- rbind(mean.results,
                          apply(do.call(rbind,lapply(mspe.cp.reps$MSPE, function(x) apply(x, 2, function(x.r) sqrt(mean(x.r^2))))), 2, mean))
