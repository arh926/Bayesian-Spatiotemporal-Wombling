load("~/Dropbox/Desktop-Files/spatiotemporal_gradient/space-time/new//application/eeg/alc_eeg.RData")
load("~/Dropbox/Desktop-Files/spatiotemporal_gradient/space-time/new//application/eeg/alc_eeg_model.RData")
require(raster)
require(sp)
require(MBA)
require(spWombling)
...
coords = alc_eeg[, c("y", "x")]
levels.curves = c(-0.5, -1, 0.5, 0.5, 0, 0)
...
curves.pm[[5]] = curves.pm[[5]][rev(1:nrow(curves.pm[[5]])),]
# original curves
...
load("~/Dropbox/Desktop-Files/spatiotemporal_gradient/space-time/new/application/eeg/wm_alc.RData")
...
WM.bytime = lapply(WM.obj$WM, function(x){
  round(rbind(t12 = apply(x[1:18,-4], 2, sum),
              ...
              t56 = apply(x[73:90,-4], 2, sum)) * 100, 2) 
})
WM.bytime
...
for(i in 1:length(WM.obj$WM)){
  ...
  rgl.snapshot(paste0(names(WM.obj$WM)[i],"_alc", fmt = ".jpg"))
}
